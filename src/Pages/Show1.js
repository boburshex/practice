...
const Show1 = () => {
  const { data2, like, sevimli, showdata, savat, bascket, minus, plus, basketPlas } = useContext(DataContext)
  const [index, setIndex] = useState(0)
  let path = useNavigate();
  const { id } = useParams();
  const [show, setShow] = useState(
    data2?.filter(item => item?.id === +id)?.length > 0 ? data2?.filter(item => item?.id === +id)[0] : false
  )
  useEffect(() => {
    if (!show) {
      path('/');
      ...
      <div className="nav-show1">
        ...
        <div className="nav-show1r">
          <div className="contShow">
            <p>On Sale from: <span>{(show.narhi ).toFixed(2)}$</span></p>
        ...
        <div className="show_sec1r">
          <div className="imgLike">
            <div className="likeicon">
              <svg onClick={() => like(show)} width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
